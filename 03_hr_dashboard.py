...
selected_member = f'{dept_text}에서 근속연수가 {min_tenure}년이상 {max_tenure}년 이하인 직원 {total_employees}'
...
아래의 데이터결과를 분석 답변
[분석대상]
조회대상 :{selected_member}
퇴직자수 : {total_attritions}명
전체 퇴직율 : {overall_rate:.1f}%
조회대상 퇴직율 : {filtered_rate:.1f}%
전체대비 조회대상 퇴직율 차이 : {rate_difference:+1f}%
...
작성규칙 :
- 분석대상, 핵심요약, 권장조치 항목 포함
-짧은 항목으로 작성, 전체 500자이내
-통계기반으로 작성
